...
public:
    int largestRectangleArea(vector<int>& heights) {
        ...
        int tempsmallestheight;
        vector<int> minheights;
        int ans = 0;
        ...
        for(int i=0; i<heights.size(); i++){
            tempsmallestheight = 1000;
            for(int j=leftsmall[i]; j<=rightsmall[i]; j++){
                tempsmallestheight = min(tempsmallestheight, heights[j]);
            }
            minheights.push_back(tempsmallestheight);
        }
        ...
        for(int i=0; i<heights.size(); i++){
            ans = max(ans, (rightsmall[i]-leftsmall[i]+1)*minheights[i]);
        }
